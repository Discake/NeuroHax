...
class PPO:
    ...
    def select_action(self, state):
        ...
        with torch.no_grad():
            state = torch.FloatTensor(state).to(Constants.device).unsqueeze(0)
    ...
    def update(self, memory : Memory, ep, logging = False):
        ...
        old_log_probs = torch.FloatTensor(memory.old_log_probs).to(Constants.device)  # От policy_old
        rewards = memory.rewards
        
        # Вычисляем returns
        returns = self.compute_returns(rewards)
        returns = torch.FloatTensor(returns).to(Constants.device)
